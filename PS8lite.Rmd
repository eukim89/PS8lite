...
```{r}
#MVP: model_rf <- randomForest(form = model_formula, data = training, mtry = 2, ntree = 100)

#Defining cv settings
fit_control <- trainControl(method = "cv", number = 10)

#Search grid for tuning parameters (mtry)
mtry <- 2:6
tunegrid <- expand.grid(.mtry = mtry)

#Fit/train model on training data
model_rf <- caret::train(form = model_formula, 
                               data = training, 
                               method = "rf", 
                               metric = "RMSE", 
...
model_rf #See which mtry value gets used
```
...
submission_rf_cv <- test %>%
  select(Id) %>%
  mutate(SalePrice = test$SalePrice_hat_rf)
write_csv(submission_rf_cv, "data/submission_rf_cv.csv")
```
